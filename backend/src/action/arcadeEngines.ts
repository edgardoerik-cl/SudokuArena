...
interface DemolitionPlayer {
  ...
  drops: Array<{ id: string; x: number; y: number; type: DemolitionDropType }>;
  laserUntil: number;
  speedUntil: number;
  wideUntil: number;
  missileUntil: number;
  missileX: number;
  nextSkyDropAt: number;
}

type DemolitionDropType =
  | "MULTIBALL_2" | "MULTIBALL_5" | "MULTIBALL_10"
  | "PADDLE_WIDE" | "MISSILE" | "LASER" | "SPEED";
...
export class DemolitionArenaEngine {
  ...
  private integrate(player: DemolitionPlayer, deltaSeconds: number): void {
    ...
    if (now >= player.nextSkyDropAt) {
      const skyTypes: DemolitionDropType[] = ["PADDLE_WIDE", "MULTIBALL_2", "MULTIBALL_5", "MULTIBALL_10", "MISSILE"];
      player.drops.push({
        id: `sky-${now}`,
        x: .08 + Math.random() * .84,
        y: -.04,
  ...
  private integrateDemolitionBall(
    ...
    if (hit) {
      ...
      if (hit.hp <= 0) {
        ...
        if (Math.random() < .24) {
          const types: DemolitionDropType[] = ["MULTIBALL_2", "LASER", "SPEED"];
          player.drops.push({
  ...
  private applyDemolitionDrop(player: DemolitionPlayer, type: DemolitionDropType, now: number): void {
    ...
    } else if (type === "PADDLE_WIDE") player.wideUntil = now + 12_000;
