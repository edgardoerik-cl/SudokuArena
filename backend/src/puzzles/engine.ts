...
export class GenericPuzzleEngine {
  ...
  private readonly lastSlitherMoveAt = new Map<string, number>();
  private readonly hangmanGuesses = new Map<string, Set<string>>();
  private readonly hangmanErrors = new Map<string, number>();
  private readonly arrowRemoved = new Map<string, Set<string>>();
  private readonly arcadeDeposits = new Map<string, string[]>();
    ...
    if (this.gameType === "ARROWS_ESCAPE") {
      ...
      for (let row = 0; row < this.board.length; row += 1) for (let col = 0; col < this.board[row]!.length; col += 1) {
        ...
        for (let y = row + dy, x = col + dx; y >= 0 && y < this.board.length && x >= 0 && x < this.board[0]!.length; y += dy, x += dx) {
          if (!removed.has(`${y}:${x}`)) { clear = false; break; }
        }
        if (clear) return { requestId: `arrows-bot-${randomUUID()}`, row, col, val: "ESCAPE" };
      }
      return null;
    }
  ...
  private applySpecificMove(
    ...
    if (this.gameType === "HANGMAN") {
      ...
      const guesses = this.hangmanGuesses.get(playerId) ?? new Set<string>();
      if (guesses.has(letter)) return { correct: false, points: 0 };
      guesses.add(letter); this.hangmanGuesses.set(playerId, guesses);
      let hits = 0;
      ...
      if (hits === 0) this.hangmanErrors.set(playerId, (this.hangmanErrors.get(playerId) ?? 0) + 1);
    ...
    if (this.gameType === "ARROWS_ESCAPE") {
      ...
      if (removed.has(key)) return { correct: false };
      ...
      for (let row = move.row + dy, col = move.col + dx;
        ...
        row += dy, col += dx) {
        if (!removed.has(`${row}:${col}`)) return { correct: false, points: 0 };
      }
      removed.add(key); this.arrowRemoved.set(playerId, removed);
    ...
    if (this.gameType === "SLITHERLINK") {
      ...
      if (String(this.answers[move.row]![move.col]) !== `${targetRow}:${targetCol}`) return { correct: false };
      const target = this.board[targetRow]?.[targetCol];
      if (!target || target.ownerId !== null) return { correct: false };
      cell.ownerId = playerId; target.ownerId = playerId;
    ...
    if (this.gameType === "RUMMIKUB") {
      ...
      if (!(zone === color || zone === "NUMBER")) return { correct: false };
  ...
  private isPuzzleComplete(): boolean {
    ...
    if (this.gameType === "HANGMAN") return this.board[0]!.every((cell) => cell.value !== null);
    ...
    if (this.gameType === "SLITHERLINK") return this.board.every((row) => row.every((cell) => cell.ownerId !== null));
    if (this.gameType === "CROSS_LETTERS") {
