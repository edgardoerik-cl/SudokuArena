...
export class GenericPuzzleEngine {
  ...
  private applySpecificMove(
    ...
    if (this.gameType === "ARROWS_ESCAPE") {
      ...
      if (action === "ROTATE") {
        ...
        const next = ({ UP: "RIGHT", RIGHT: "DOWN", DOWN: "LEFT", LEFT: "UP" } as Record<string, string>)[String(shape.direction)] ?? "UP";
      ...
      const opposite = ({ UP: "DOWN", DOWN: "UP", LEFT: "RIGHT", RIGHT: "LEFT" } as Record<string, string>)[primary]!;
  ...
  private applyChessTacticsMove(
    ...
    if (skill === "PHALANX_CHARGE") {
      ...
      const revived: Piece = { ...piece, id: `${piece.team}-PAWN-revived-${Date.now()}`, type: "PAWN", currentCooldown: 0, statusEffects: [], isShielded: false };
  ...
  private isPuzzleComplete(): boolean {
    ...
    if (this.gameType === "NEXUS_ZERO") return this.board.flat().every((cell) => cell.value === null);
