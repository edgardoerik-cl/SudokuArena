...
io.on("connection", (socket) => {
    ...
    socket.on("pause:respond", (payload) => {
        ...
        if (payload?.accepted === true)
            ...
            room.pauseNoVotes.add(playerId);
        emitRoomState(room);
        maybeActivatePause(room);
    });
    ...
    socket.on("demolition:input", (payload) => {
        const room = roomFor(socket);
        if (!room || room.phase !== "PLAYING" || room.config.gameType !== "DEMOLITION_ARCADE" || !room.demolitionEngine)
            return;
...
function removeDisconnectedPlayer(room, playerId) {
    ...
    if (room.game.humanPlayerCount === 0) {
        ...
        if (room.demolitionTick)
            clearInterval(room.demolitionTick);
        clearBotTimers(room);
...
function processGenericMove(room, playerId, payload, responder) {
    ...
    try {
        ...
        if (room.config.gameType !== "ARROWS_ESCAPE")
            emitGenericState(room);
        return;
    }
    if (!result.accepted) {
        ...
        if (room.config.gameType === "CHECKERS")
            responder?.emit("InvalidMove", result);
...
function emitGenericState(room, compact = false) {
    if (!room.genericEngine)
        return;
...
function maybeActivatePause(room) {
    ...
    if (!room.pauseRequesterId || humans.length === 0)
        return;
    ...
    if (!approved) {
        if (allVoted)
            cancelPauseRequest(room, "La mayoría rechazó la pausa");
        return;
    }
...
function startDemolitionLoop(room) {
    if (!room.demolitionEngine || room.demolitionTick)
        return;
    let emittedTick = 0;
    room.demolitionTick = setInterval(() => {
        if (rooms.get(room.code) !== room || room.phase !== "PLAYING" || !room.demolitionEngine)
            return;
