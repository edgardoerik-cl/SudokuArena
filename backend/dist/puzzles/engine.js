...
export class GenericPuzzleEngine {
    ...
    towerProjectiles = [];
    towerLastTickAt = 0;
    towerWaveStartedAt = 0;
    towerNextShotAt = new Map();
    towerWaveKills = new Map();
    reactorPowerEnergy = new Map();
    mergeBotStep = 0;
    ...
    createBotMove(accuracy, playerId) {
        ...
        if (this.gameType === "TIC_TAC_TOE") {
            let forced = this.meta.forcedMini;
            const miniWinners = (this.meta.miniWinners ?? {});
            if (forced) {
                ...
                if (miniWinners[forcedKey] || !hasFreeCell) {
                    forced = null;
                    this.meta.forcedMini = null;
                }
            }
            const candidates = [];
            for (let row = 0; row < this.board.length; row += 1)
                for (let col = 0; col < this.board[row].length; col += 1) {
                    if (forced && !miniWinners[`${forced.row}:${forced.col}`]
                        && (Math.floor(row / 3) !== forced.row || Math.floor(col / 3) !== forced.col))
                        continue;
                    if (miniWinners[`${Math.floor(row / 3)}:${Math.floor(col / 3)}`])
                        continue;
    ...
    applySpecificMove(playerId, move, cell, game, now) {
        ...
        if (this.gameType === "TIC_TAC_TOE") {
            ...
            if (this.meta.variant === "CLASSIC") {
                if (cell.value !== null)
                    return { correct: false, neutral: true, message: "Casilla ocupada" };
                cell.value = mark;
                cell.ownerId = playerId;
                cell.isRevealed = true;
            ...
            const usedPowers = (this.meta.ticUsedPowers ?? {});
            const mine = new Set(usedPowers[playerId] ?? []);
            if (["PUSH", "SHIELD", "BOMB"].includes(action)) {
                if (mine.has(action))
                    return { correct: false, neutral: true, message: "Poder ya utilizado" };
                if (action === "SHIELD") {
                    if (cell.ownerId !== playerId || cell.value !== mark)
                        return { correct: false };
                    cell.meta.shielded = true;
                }
                else if (action === "BOMB") {
                    if (cell.value === null || cell.meta.shielded === true)
                        return { correct: false };
                    cell.value = null;
                    cell.ownerId = null;
                    ...
                    if (cell.value === null || cell.ownerId === playerId || cell.meta.shielded === true)
                        return { correct: false };
                    ...
                    if (!destination)
                        return { correct: false, neutral: true, message: "No hay casilla adyacente libre" };
                    destination.value = cell.value;
                    destination.ownerId = cell.ownerId;
                    destination.isRevealed = true;
                ...
                mine.add(action);
                usedPowers[playerId] = [...mine];
                this.meta.ticUsedPowers = usedPowers;
            ...
            if (miniWinners[selectedMiniKey])
                return { correct: false, neutral: true, message: "Ese mini-tablero ya fue conquistado" };
            if (forced) {
                valLoop: {
                    const region = this.board.slice(forced.row * 3, forced.row * 3 + 3).flatMap((row) => row.slice(forced.col * 3, forced.col * 3 + 3));
                    if (!miniWinners[`${forced.row}:${forced.col}`] && region.some((candidate) => candidate.value === null))
                        break valLoop;
                    forced = null;
                    this.meta.forcedMini = null;
                }
            ...
            if (owned.length > 3) {
                ...
                oldest.value = null;
                oldest.ownerId = null;
                oldest.isRevealed = false;
                oldest.meta.placedAt = null;
            }
            const miniRow = Math.floor(move.row / 3);
            const miniCol = Math.floor(move.col / 3);
            const miniWinner = this.ticRegionWinner(miniRow * 3, miniCol * 3);
            if (miniWinner) {
                miniWinners[`${miniRow}:${miniCol}`] = miniWinner;
                const miniOwners = (this.meta.miniOwners ?? {});
        ...
        if (this.gameType === "ARROWS_ESCAPE") {
            ...
            const shape = this.meta.shapes.find((candidate) => candidate.id === shapeId);
            const triggered = this.arrowTimerTriggered.get(playerId) ?? new Set();
            this.arrowTimerTriggered.set(playerId, triggered);
            const expiredTimers = this.meta.shapes.filter((candidate) => candidate.blockType === "TIMER"
                && !candidate.memberKeys.every((key) => removed.has(key))
            ...
            if (shape?.blockType === "BOMB") {
                const sourcePoints = shape.points;
                const sourceHead = sourcePoints?.[sourcePoints.length - 1];
                const sourceX = Number(sourceHead?.x ?? shape.x ?? 0);
                const sourceY = Number(sourceHead?.y ?? shape.y ?? 0);
                const candidates = this.meta.shapes
                    ...
                    const aHead = aPoints?.[aPoints.length - 1];
                    const bHead = bPoints?.[bPoints.length - 1];
        ...
        if (this.gameType === "HITORI" && typeof move.val === "object" && move.val !== null
            && String(move.val.action).toUpperCase() === "HINT") {
            ...
            if (!safe)
                return { correct: false, neutral: true, message: "No quedan deducciones pendientes" };
        ...
        if (this.gameType === "NURIKABE") {
            ...
            if (String(payload?.action ?? "").toUpperCase() === "SONAR") {
                ...
                if (used >= 3)
                    return { correct: false, neutral: true, message: "No quedan pulsos de Sonar" };
    ...
    ticTacToeWinner() {
        if (this.board.length === 9) {
            const miniWinners = (this.meta.miniWinners ?? {});
            if (Object.keys(miniWinners).length < 9)
                return null;
    ...
    ticRegionWinner(startRow, startCol) {
        const region = Array.from({ length: 3 }, (_, row) => Array.from({ length: 3 }, (_, col) => String(this.board[startRow + row][startCol + col].value ?? "")));
        const lines = [
            [[0, 0], [0, 1], [0, 2]], [[1, 0], [1, 1], [1, 2]], [[2, 0], [2, 1], [2, 2]],
            [[0, 0], [1, 0], [2, 0]], [[0, 1], [1, 1], [2, 1]], [[0, 2], [1, 2], [2, 2]],
            [[0, 0], [1, 1], [2, 2]], [[0, 2], [1, 1], [2, 0]],
    ...
    spawnNexusWave(pairCount) {
        ...
        for (let pair = 0; pair < Math.min(pairCount, Math.floor(this.board.length * this.board.length / 2)); pair += 1) {
            ...
            used.add(`${row}:${first.col}`);
            used.add(`${row}:${second.col}`);
    ...
    tickTowerDefense(game, now = Date.now()) {
        ...
        if (this.towerWaveStartedAt > 0 && now - this.towerWaveStartedAt >= 75_000) {
            ...
            if (stalled.length)
                this.towerBaseHealth = Math.max(0, this.towerBaseHealth - 1);
        }
    ...
    applyTowerDefenseMove(playerId, move, game) {
        ...
        if (["EMP", "ORBITAL", "REPAIR", "OVERCLOCK"].includes(action)) {
            const costs = { EMP: 180, ORBITAL: 280, REPAIR: 220, OVERCLOCK: 200 };
            const cost = costs[action];
            const credits = this.towerCredits.get(playerId) ?? 0;
            if (credits < cost)
                return { correct: false, neutral: true, message: "Créditos insuficientes para la habilidad" };
            ...
            this.board.flat().forEach((tower) => {
                if (tower.ownerId === playerId && tower.meta.towerType != null)
                    tower.meta.overclockUntil = now + 12_000;
            });
    ...
    applyReactorChainMove(playerId, move) {
        ...
        if (specialTarget && embeddedPower) {
            ...
            if (embeddedPower === "RAINBOW") {
                ...
                this.board.flat().forEach((cell) => {
                    if (Number(cell.value) === sourceColor) {
                        cell.value = 0;
                        cell.meta = { reactorOrb: true, special: "WILD" };
                    }
                });
            ...
            if (!this.hasReactorMove()) {
                this.meta.noMoves = true;
                this.completed = true;
            }
        ...
        const costs = { HAMMER: 3, ROW_BLAST: 4, COLOR_WIPE: 5, SHUFFLE: 3 };
        if (action in costs) {
            const cost = costs[action];
            if (currentEnergy < cost)
                return { correct: false, neutral: true, message: "EnergÃ­a de reactor insuficiente" };
            ...
            if (!selected)
                return { correct: false, neutral: true, message: "Objetivo invÃ¡lido" };
        ...
        if (createdSpecial) {
            ...
            if (survivor) {
                survivor.value = color;
                survivor.meta = { reactorOrb: true, special: createdSpecial, sourceColor: color };
            }
        }
    ...
    removeAndCollapseReactors(cells, playerId) {
    ...
    ensureReactorMove() {
        ...
        if (!playable) {
            ...
            for (const [row, col] of [[0, 0], [0, 1], [1, 0]])
                this.board[row][col].value = color;
        }
    }
    ...
    applyChessTacticsMove(playerId, move, source) {
        ...
        if (action === "MOVE" || action === "ATTACK") {
            ...
            if (piece.type === "PAWN" && !expectsAttack) {
                ...
                if (targetCol !== move.col || distance !== direction && distance !== direction * 2)
                    return { correct: false };
        ...
        if (skill === "PHALANX_CHARGE") {
            ...
            if ((this.capturedPawns.get(piece.team) ?? 0) <= 0)
                return { correct: false };
            const revived = { ...piece, id: `${piece.team}-PAWN-revived-${Date.now()}`, type: "PAWN", currentCooldown: 0, statusEffects: [], isShielded: false, hasMoved: true };
    ...
    canArrowShapeEscape(shapeId, removed, directionOverride) {
        if (this.meta.pathModel === "SERPENTINE_V2") {
            ...
            const rayBlockedBy = (obstacle) => {
                for (let index = 1; index < obstacle.points.length; index += 1) {
                    const start = obstacle.points[index - 1];
                    const end = obstacle.points[index];
                    for (let step = 1; step <= 32; step += 1) {
                        const t = step / 32;
                        const point = { x: head.x + (rayEnd.x - head.x) * t, y: head.y + (rayEnd.y - head.y) * t };
                        if (pointToSegment(point, start, end) <= route.thickness + obstacle.thickness + .008)
                            return true;
                    }
                }
                return false;
            };
            return !routes.some((obstacle) => obstacle.id !== route.id
    ...
    isPuzzleComplete() {
        ...
        if (this.gameType === "TIC_TAC_TOE")
            return this.meta.variant === "CLASSIC"
    ...
    applyCrossLettersMove(playerId, move) {
        ...
        if (!SPANISH_WORDS.has(word) || !["H", "V"].includes(direction) || word.length < 2 || [...word].length > 15)
            return { correct: false };
        ...
        for (const { row, col, letter } of coordinates) {
            ...
            if (fresh) {
                ...
                target.ownerId = playerId; // Word Conquest: reutilizar una letra reconquista la casilla.
        }
        const entry = SPANISH_DICTIONARY.find((candidate) => normalizeSpanishWord(candidate.word) === word);
        const played = (this.meta.playedWords ?? []);
