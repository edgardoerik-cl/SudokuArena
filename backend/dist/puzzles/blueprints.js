...
function nexusZero(random, difficulty) {
    ...
    const partner = Array.from({ length: rows * columns }, (_, index) => index % columns % 2 === 0 ? index + 1 : index - 1);
...
function connectDotsNeon(random, difficulty) {
    const size = sizeFor(difficulty, 5, 6, 7, 8);
    const order = [];
    for (let row = 0; row < size; row += 1) {
...
function rayCells(row, col, direction, size) {
    const [dy, dx] = direction === "UP" ? [-1, 0] : direction === "RIGHT" ? [0, 1]
        : direction === "DOWN" ? [1, 0] : [0, -1];
    const result = [];
    for (let y = row + dy, x = col + dx; y >= 0 && y < size && x >= 0 && x < size; y += dy, x += dx)
        result.push([y, x]);
    return result;
}
