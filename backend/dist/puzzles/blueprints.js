...
function wordSearch(random, difficulty) {
    ...
    const answers = matrix(size, size, () => random.pick([...letters]));
...
function logicTiles(random, difficulty) {
    ...
    const answers = matrix(rows, columns, (row, col) => ((row * 3 + col * 2 + random.int(0, 4)) % 9) + 1);
    const board = matrix(rows, columns, (row, col) => {
        const expected = answers[row][col];
        const operation = random.pick(operations);
        const left = Math.max(0, expected - 1);
        const right = expected + 1;
