const LATIN_9 = Array.from({ length: 9 }, (_, row) => Array.from({ length: 9 }, (_, column) => ((row * 3 + Math.floor(row / 3) + column) % 9) + 1));
export function createPuzzleBlueprint(gameType) {
    switch (gameType) {
...
function wordSearch() {
    ...
    const answers = matrix(10, 10, (row, col) => row < words.length && col < words[row].length ? words[row][col] : filler[(row * 7 + col * 11) % filler.length]);
...
function dotsAndBoxes() {
    ...
    const answers = matrix(5, 5, () => true);
    return { board, answers, meta: { dots: 6 } };
}
function numberPuzzle(kind, size) {
...
function transpose(matrixValue) {
